...
- F5: every config-gate test explicitly pins the env — `monkeypatch.delenv("AI_BADGER_MEMORY_GRADE", raising=False)` for OFF paths, `monkeypatch.setenv("AI_BADGER_MEMORY_GRADE", "1")` for ON paths — independent of the ambient environment (WP7 enables it machine-wide on this host; the suite must not inherit that).
...
- RED: `test_search_call_appends_one_line_with_all_fields` (args `{projectId, query, scope, workspaceId}`, result JSON → line has `ts/query/scope/projectId/workspaceId/result/usefulness:null/note:null`), `test_missing_workspace_id_logs_null`, `test_result_not_json_logs_raw_payload`, `test_non_search_tool_writes_nothing`, `test_disabled_config_writes_nothing`, `test_line_shape_matches_manual_jsonl` (top-level keys are a superset of the two lines in `docs/work/2026-08-05-ai-raccoon-memory-quality.jsonl` — `ts, query, scope, projectId, result, usefulness, note`; result items vary between lines — `score` vs `hash/ranking` — so the hook preserves the payload verbatim).
- F4: the hook reads args defensively for BOTH spellings — `project_id`|`projectId`, `workspace_id`|`workspaceId`, `query`, `scope` (the tool's documented interface is `memory_search(project_id, scope=all)`; the MCP arg names are otherwise unverifiable in-repo). The WP2 test exercises both spellings; workspace stays null when absent.
- F8: the sibling-injection must be explicit — `_load_memory_grade` returns None in the framework checkout (the script lives under `features/common/skills/ai-raccoon-memory/scripts/`, never beside `ai_badger_hooks.py`), so the tests inject the real module into `sys.modules` under the module-name constant, exactly like `test_commit_reminder_hermes.py:61` (`monkeypatch.setitem(sys.modules, hooks.COMMIT_REMINDER_MODULE_NAME, module)` via `load_script`). Also monkeypatch the module-level log/pending path constants to `tmp_path` (same as test_commit_reminder_hermes.py:28).
- Acceptance: exactly one line per search; full result preserved; log file created lazily (mkdir -p); the new section is exception-guarded and never raises (same discipline as `_maybe_remind_commit`).
...
- RED: `test_search_stashes_pending_ask_keyed_by_project`, `test_ask_injected_once_on_next_pre_llm_call` (ask text present in `result["context"]` — the key pre_llm_inject_context returns, ai_badger_hooks.py:526), `test_ask_not_injected_a_second_time` (pop-once), `test_ask_injection_is_inert_when_disabled`, `test_ask_contains_helper_command_and_ts`.
- F5: OFF-path tests pin `monkeypatch.delenv("AI_BADGER_MEMORY_GRADE", raising=False)`; ON-path tests `setenv(..., "1")`.
- F8: inject the sibling into `sys.modules` under the module-name constant (as above); monkeypatch pending-path constant to `tmp_path`.
...
**WP5 — Claude hook + manifest wiring.** Files: `tests/test_memory_grade_claude_hook.py`, `tests/test_hooks_manifest_agent_coverage.py` (passes only once the manifest names all three agents), `tests/test_commit_reminder_wiring.py` (copilot test must tolerate the new entry — see F1), `features/common/skills/ai-raccoon-memory/scripts/memory_grade_hook.py`, `features/common/hooks/hooks-manifest.json`, `features/common/hooks/hooks.json`.
- RED: `test_claude_hook_stdin_payload_appends_line_and_emits_additional_context` (drive `main()` with a `tool_name=tool_input=tool_response` payload), `test_claude_hook_silent_on_non_search_or_disabled`, `test_memory_grade_manifest_names_all_three_agents` (a NEW presence test — the existing gap test stays green while the entry is absent, because validate.py iterates only hooks present in the manifest; F3), `test_hooks_json_post_tool_use_wires_the_script` (wiring shape check).
- F1 (mandatory): `tests/test_commit_reminder_wiring.py::test_copilot_wires_commit_reminder_with_matcher` breaks once the memory-grade manifest entry + hooks.json PostToolUse entry exist — the copilot rewrite wires every manifest copilot entry whose command ends with the named script, and the test asserts `len(post_tool_use) == 1` with `skills=["commit-reminder"]`. Update it to assert the commit-reminder matcher entry is *present* among the wired postToolUse entries (not an exact count), and add the file to this WP's gate.
- Acceptance: Claude hook is advisory-only (exit 0, `additionalContext` only); the real manifest names all three agents for memory-grade; the rewrite source wires the script with the `memory_search` matcher.
- Gate: `python3 -m pytest -q tests/test_memory_grade_claude_hook.py tests/test_hooks_manifest_agent_coverage.py tests/test_commit_reminder_wiring.py tests/test_hook_wiring_claude.py && python3 tooling/validate.py --all`.

**WP6 — Deployment + docs + release.** Files: `tests/test_hermes_plugin_install.py` (extend `SHARED_SKILL_FILES`; **also generalize `_commit_reminder_script`** — F2: it hardcodes `features/common/skills/commit-reminder/scripts/<name>`, so adding `"memory_grade.py"` to the constant makes the existing loop read `commit-reminder/scripts/memory_grade.py` → FileNotFoundError; resolve the skill dir per `(skill, filename)` tuple instead), `features/hermes/adjustments/adjust_hooks.py`, `features/common/skills/ai-raccoon-memory/SKILL.md`, `VERSION` (0.78.0 → 0.79.0), `docs/changelog/0.79.0-memory-grade-hook.md`.
