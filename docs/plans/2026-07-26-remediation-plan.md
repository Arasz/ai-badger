...
| **WP16** | F-24 — drive `validate_all` from the schemas directory; replace `mcp_tools.py`'s two silent `continue`s with notes | Important | `scripts/validate.py`, `features/common/skills/welcome-ai-badger/scripts/mcp_tools.py`, `tests/test_validate.py` |
...
## Hard ordering constraints (violating these introduces a defect)
...
| `scripts/badger_lib.py` | WP12, WP13 (both Wave 3) | **Serialise WP12 → WP13** inside Wave 3, or give one agent both. |
