...
**Owner refinement (2026-09-01, live observation):** mail must surface BETWEEN tasks, not after a cancel. pi's lifecycle (docs/extensions.md:285-318) has a per-turn injection seam INSIDE the agent loop — `context` ("can modify messages") fires every turn while the run loops on tool calls. P4 therefore wires TWO delivery points: `before_agent_start` (run start, unconditional live read) AND `context` (per-turn live read, mail appended via the existing injection shape). The `tool_call` seam can only block, not inject — not usable. `agent_settled` is the run-complete marker (no delivery role).
...
| R9 | **Self-delivery via sender-identity drift** (owner-observed live 2026-09-01 11:15Z): the send CLI resolved the jsaa agent's sender to its task-tracked session id (`d9e90bb1…`, user-DB msg id 11, machine broadcast), while the live harness session (`01a05c6d…`) received it — the R2 exclusion compares recorded ids, and one agent carried two. Fix is SEND-side identity derivation (`send_message.py` — not vendored, no store-lane collision): derive the harness session id the way delivery does (payload/env first), or exclude by project+recency shape. **New package P9, TDD red test: an agent's own broadcast never re-delivers to that agent across the id-drift shape.** Evidence: user-DB cursors show `01a05c6d…` consumed id 11 at 11:17:31Z; `d9e90bb1…` has no cursor row. |
