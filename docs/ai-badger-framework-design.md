...
## 2. Architecture: `{stack | common}/{feature}`

The framework repo is organized as **stack × feature**:

- **stack** — a technology: `dotnet`, `azure`, `cosmos`, `terraform`, `mcp`, `node`, `js`,
  `ts`, `react`, `css`, `github`, … plus **`common`** for stack-agnostic content.
- **feature** — a kind of framework asset: **`skills`**, **`personas`**, **`invariants`**,
  **`instructions`**, **`plugins`**. (Marketplaces are merged into `plugins` — see §6.)
...
ai-badger/
  ...
  plugin.json / plugins/…           # (the installable plugin wrapping common skills — see §9)
...
markdown; skills are directories; plugins entries are directories (§6).
...
```jsonc
{
  ...
  "stacks": {
    "common": {
      ...
      "invariants":   [ { "name": "tdd", "path": "common/invariants/tdd.md" }, … ],
      "instructions": [ … ],
      "plugins":      [ { "name": "superpowers", "path": "common/plugins/superpowers" }, … ]
    },
    "dotnet": { "personas": [ { "name": "dotnet-engineer", "path": "dotnet/personas/dotnet-engineer.md" } ], "invariants": […], "instructions": […], "plugins": […] },
...
| 5 | Repo layout | `{stack\|common}/{feature}` (§2) |
| 6 | Plugins | marketplaces merged into `plugins` feature; each entry refs `marketplaces.json`; scope default(inherit)/local/user |
...
- In `feed`: classify candidates agnostic/generalizable/project-specific, generalize wording,
  choose target stack/feature placement.

## 6. `plugins` feature (marketplaces merged in)

A plugins entry = a directory under `<stack>/plugins/<name>/` containing:
- `plugin.json` — `{ name, scope: "default"|"local"|"user", plugins: [names to install] }`
  (`schemas/plugin-entry.schema.json`). **default = inherit the current scope** chosen at init.
- `marketplaces.json` — the custom marketplaces to add for this entry
  (`schemas/marketplaces.schema.json`), e.g. `{ "marketplaces": [ { "name": "…", "source": "github:Owner/repo" } ] }`.
...
- **Base `task`** (`skills/task`): orchestration, Fable/Sonnet/Haiku delegation, TDD,
  token tracking, resume cron, finish protocol, review loop — all reading `config.json`.
  No hardcoded GitHub, no dashboard, no `dotnet`.
- **Extensions** (`<stack>/skills/task-extensions/<name>` or `github/skills/…`): opt-in slices
  embedded into the scaffolded skill when config supplies their data:
...
- `prompt-markers` (ADR-0017) ships as its own `common` skill (hook + `markers-context.json` +
  `marker-state.json`), referenced by base task.
...
## 9. Installable plugin + marketplace

`ai-badger` is itself a Claude marketplace: `.claude-plugin/marketplace.json` → one plugin whose
skills are the `skills` (`welcome-ai-badger`, `feed-badger`, `task`,
`maintain-agent-instructions`, `auto-wm`, `prompt-markers`). Install:
`/plugin marketplace add Arasz/ai-badger` → `/plugin install ai-badger` → run `welcome-ai-badger`.
(The plugin references the `skills` dirs; catalog stacks are data the skills read.)
...
2. Agent classifies + generalizes + places into `{stack}/{feature}`.
