...
| `vendor_archify.py` | Keep the vendored Archify packet honest: an offline `--check` compares the tree against `vendor.json` (MISSING/EXTRA/CHANGED), and `--revendor <archify.zip> --expect-sha256 <sha>` re-vendors a new upstream release with the expected sha supplied on the command line. The origin check is `--revendor`; `--check` proves internal consistency only. | `python3 tooling/vendor_archify.py --check` · maintainer re-vendor: `gh release download v2.16.0 -p archify.zip` then `python3 tooling/vendor_archify.py --revendor archify.zip --expect-sha256 4c59fa65…`. |
