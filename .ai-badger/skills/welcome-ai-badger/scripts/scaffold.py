...
class Scaffolder:
    ...
    def scaffold_gitignore(self) -> None:
        ...
        Merged, never owned: entries outside the markers are the project's. Unbalanced
        markers are reported, not rewritten — half a block means a hand edit is in flight.
        """
        path = self.target / ".gitignore"
        try:
            text = path.read_text(encoding="utf-8") if path.exists() else ""
        except UnicodeDecodeError:
            self.notes.append(".gitignore is not UTF-8 text — managed block not written")
            return
        merged = merge_gitignore(text)
        if merged is None:
            self.notes.append(".gitignore has an unbalanced ai-badger block marker — "
                              "managed block not rewritten; fix the markers by hand")
            return
        if merged != text:
            path.write_text(merged, encoding="utf-8")
            self.notes.append("merged managed gitignore block (task-tracking SQLite artifacts)")
            self.ctx.record_generated_config(path, ".gitignore")
