...
plugins/memory/__init__.py, 2026-08-06) and a working provider implementation.
...
- Provider name == directory name. A hyphenated dir (e.g. `the project`) loads fine at
...
The Hermes source checkout (`~/.hermes/hermes-agent/`) is the primary reference:
`agent/memory_provider.py`, `agent/memory_manager.py`, `plugins/memory/__init__.py`.
Keep a project-specific bridge record (ratified design decisions, tool contracts +
result shapes, isolation incidents, smoke recipes) beside the provider when the
integration outgrows this file.
