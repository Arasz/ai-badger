...
# ------------------------------------------------- grounded-feedback-failure arm (P11, D5=A)
GROUNDED_SCRIPT = "grounded_feedback_hook.py"


def test_manifest_adds_a_second_claude_only_entry_for_the_failure_event(root):
    """The existing `grounded-feedback` entry and its PostToolUse arm are untouched; a second,
    claude-only entry wires the same script onto PostToolUseFailure."""
    manifest = json.loads(
        (root / "features" / "common" / "hooks" / "hooks-manifest.json")
        .read_text(encoding="utf-8"))
    by_name = {hook["name"]: hook for hook in manifest["hooks"]}

    assert "grounded-feedback" in by_name
    original_claude = by_name["grounded-feedback"]["agents"]["claude"]
    assert original_claude["event"] == "PostToolUse"
    assert original_claude["script"] == GROUNDED_SCRIPT

    assert "grounded-feedback-failure" in by_name
    failure_agents = by_name["grounded-feedback-failure"]["agents"]
    assert set(failure_agents) == {"claude"}
    failure_claude = failure_agents["claude"]
    assert failure_claude["event"] == "PostToolUseFailure"
    assert failure_claude["script"] == GROUNDED_SCRIPT
    assert failure_claude["type"] == "hooks-json"


def test_hooks_json_post_tool_use_failure_wires_grounded_feedback(root):
    ...
    commands = [h["command"] for e in entries for h in e["hooks"] if GROUNDED_SCRIPT in h["command"]]
    assert commands, hooks_json["hooks"].get("PostToolUseFailure")


def test_scaffold_wires_grounded_feedback_into_both_post_events(tmp_path, load_script, root):
    ...
    target = tmp_path / "proj"
    _skill_scripts(target, "prompt-markers", GROUNDED_SCRIPT)
    manifest_hooks = json.loads(
        (root / "features" / "common" / "hooks" / "hooks-manifest.json")
        .read_text(encoding="utf-8"))["hooks"]
    source_hooks = _hooks_json(root)
    ...
    for event in ("PostToolUse", "PostToolUseFailure"):
        fired = [h["command"] for e in settings["hooks"].get(event, []) for h in e["hooks"]
                 if GROUNDED_SCRIPT in h["command"]]
        assert fired, (event, settings["hooks"].get(event))


def test_pi_still_reads_grounded_feedback_from_generated_post_tool_use(tmp_path, load_script,
                                                                        root):
    """pi's adapter dynamically reads .ai-badger/hooks/hooks.json's PostToolUse group and runs
    it for both a successful and a failed tool_result (plan-v2.md P11's pi-preservation pin,
    R12) — adding the claude-only failure arm must never move grounded-feedback off PostToolUse,
    only add a second, separate entry under PostToolUseFailure."""
    target = tmp_path / "proj"
    _skill_scripts(target, "prompt-markers", GROUNDED_SCRIPT)
    manifest_hooks = json.loads(
        (root / "features" / "common" / "hooks" / "hooks-manifest.json")
