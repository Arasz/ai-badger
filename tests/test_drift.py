...
def test_nested_entry_does_not_make_new_parent_directories_local_modification(
        tmp_path, load_script):
    """A nested entry's newly created directory chain is outside the parent skill's ownership."""
    drift = load_script("features/common/skills/welcome-ai-badger/scripts/drift.py")
    bl = load_script("engine/badger_lib.py")
    fw, proj, manifest = _mcp_index_shaped(tmp_path, bl)
    ...
    manifest["entries"].append({
        "feature": "adjustments", "stack": "claude", "name": "generated/file.py",
        "source": "features/claude/adjustments/adjust_retrieval.py",
        "target": ".ai-badger/skills/mcp-index/generated/newdir/file.py",
        "hash": "0" * 64,
    })
