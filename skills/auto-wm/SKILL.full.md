...
- **Denylist.** Destructive shell commands (`rm -r`, `sudo`, force-pushes, `reset --hard`, `crontab`, …), network egress (`WebFetch`, `WebSearch`, `curl`/`wget`/`ssh`-style clients, and every MCP tool), writes outside the project (including shell redirections), and changes to away-mode state (any `awm.py` subcommand except `status` and `decision`) are never auto-approved. Flags are judged word by word, so their order and spelling do not matter, and any tool whose input carries a shell command is scanned, not only Bash. They fall through to the normal permission prompt and are logged as `denylisted`. AWM refuses to arm at `/` or at `$HOME`, and the gate ignores a window recorded there.
