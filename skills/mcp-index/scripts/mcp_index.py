...
_DEFAULT_TAXONOMY: dict[str, Any] = {
    "categories": {
        "language": {"tags": ["csharp", "typescript", "javascript", "python", "sql", "css", "html"]},
        "action": {"tags": ["navigation", "diagnostic", "build", "run", "refactoring", "search", "read", "write", "terminal"]},
        "domain": {"tags": ["database", "tracing", "opentelemetry", "browser", "dotnet", "semantic", "files"]},
...
def _auto_tags(tool_name: str, server_name: str = "") -> list[str]:
    ...
    if "sql" in name or "database" in name or "schema" in name or "db" in name:
    ...
    if "search" in name or "find" in name:
        tags.add("search")
    if "refactor" in name or "rename" in name or "reformat" in name or "move_type" in name:
        tags.add("refactoring")
    if "symbol" in name:
        tags.update(["semantic", "search"])
    if "problem" in name or "error" in name or "diagnostic" in name:
        tags.add("diagnostic")
    if "span" in name or "trace" in name or "log" in name or "otel" in name or ("service" in name and "map" not in name):
        tags.update(["tracing", "opentelemetry"])
    if "service_map" in name:
        tags.update(["tracing", "opentelemetry"])
    if "browser" in name or "navigate" in name or "screenshot" in name or "click" in name:
        tags.add("browser")
    if "file" in name or "directory" in name or "tree" in name or "glob" in name:
        tags.add("files")
    ...
    if "read" in name:
        tags.add("read")
    if "write" in name or "create" in name or "patch" in name or "replace" in name:
        tags.add("write")
    if "terminal" in name or "shell" in name:
        tags.add("terminal")

    # Server cleanup: restrict to valid taxonomy tags
...
def _fetch_mcp_tools(from_json: Optional[str] = None) -> list[dict[str, Any]]:
    ...
    if result.returncode != 0:
        print(f"ERROR: hermes mcp list --json failed: {result.stderr}", file=sys.stderr)
        sys.exit(1)
...
def cmd_init(target: str, from_json: Optional[str] = None) -> int:
    ...
    for server in servers:
        ...
        for tool in server.get("tools", []):
            ...
            tools[name] = {
                ...
                "intent": tool.get("description", f"TODO: describe what {name} does"),
            }
    ...
    general_count = sum(
        1 for s in sources for t in s["tools"].values() if t["tags"] == ["general"]
    )
    print(f"Indexed {tool_count} tools across {len(sources)} server(s).")
    if general_count:
        print(f"  {general_count} tool(s) tagged as 'general' — run 'mcp-index tag <tool> <tags...>' to curate.")
    return 0
...
def cmd_validate(target: str) -> int:
    ...
    if index is None:
        print("ERROR: .ai-badger/mcp-tools.yaml not found. Run 'mcp-index init' first.", file=sys.stderr)
        return 1
    ...
    for source in index.get("sources", []):
        ...
        for tname, tool in source.get("tools", {}).items():
            ...
            if not tags:
                ...
                errors.append(f"{full}: still tagged 'general' — needs curation")
            else:
            ...
            if not intent:
                ...
                errors.append(f"{full}: intent too short ({len(intent)} chars, min 10)")
...
def cmd_update(target: str, from_json: Optional[str] = None) -> int:
    ...
    for server in servers:
        current_tools[server["name"]] = {t["name"] for t in server.get("tools", [])}
    ...
    for source in index.get("sources", []):
        ...
        server_data = next((s for s in servers if s["name"] == sname), None)
        for name in added:
            desc = ""
            if server_data:
        ...
        for server in servers:
            if server["name"] not in {s["name"] for s in index["sources"]}:
                tools = {}
                for tool in server.get("tools", []):
                    tools[tool["name"]] = {
                        "tags": _auto_tags(tool["name"], server["name"]),
                        "intent": tool.get("description", f"TODO: describe what {tool['name']} does"),
                    }
...
def cmd_tag(target: str, tool_ref: str, tags: list[str]) -> int:
    ...
    invalid = [t for t in tags if t not in all_tags]
    if invalid:
        print(f"ERROR: invalid tags: {invalid}. Valid tags: {sorted(all_tags)}", file=sys.stderr)
        return 1
    ...
    if ":" not in tool_ref:
        print("ERROR: tool reference must be in format 'server:tool_name'.", file=sys.stderr)
        return 2
...
def cmd_intent(target: str, tool_ref: str, intent: str) -> int:
    ...
    if len(intent) < 10:
        print(f"ERROR: intent must be at least 10 characters (got {len(intent)}).", file=sys.stderr)
        return 1
...
def cmd_list(target: str, tag: Optional[str] = None, untagged: bool = False) -> int:
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        ...
        # Remove --target and its value from argv for sub-command parsing
        remaining = [a for i, a in enumerate(argv) if i not in (target_idx, target_idx + 1)]
    ...
    if cmd == "tag":
        # tag <tool_ref> <tag...> [--target <path>]
        args = [a for a in remaining[1:] if a != "--target" and a not in (
            remaining[remaining.index("--target") + 1] if "--target" in remaining else []
        )]
        # args that aren't --target values
        clean_args = []
        skip = False
        for a in remaining[1:]:
            ...
            clean_args.append(a)
        if len(clean_args) < 2:
            print("ERROR: tag requires tool_ref and at least one tag.", file=sys.stderr)
            return 2
    ...
    if cmd == "intent":
        ...
        for a in remaining[1:]:
            ...
            clean_args.append(a)
        if len(clean_args) < 2:
            print("ERROR: intent requires tool_ref and intent text.", file=sys.stderr)
            return 2
    ...
    if cmd == "list":
        ...
        return cmd_list(target, tag=tag_filter, untagged="--untagged" in remaining)
