...
From implementing the project provider (MemoryProvider shim over the
the MCP server, 2026-08-06, PR #61). Interface background:
...
3. **Provider dir name = provider name, and it may be unhyphenatable**
   (`the project`). Python cannot `import the project`; tests must spec-load
...
- Integration (slow, `--run-slow` marker): spawn the REAL installed binary
  via the provider's stdio transport with `<APP>_DATA_ROOT=<tmp>`
