...
Case: `the maintainer.the project` → `the project` (the reference repo, PR #53). NuGet support
assigned the raw id one day after the support email about an ownerless reserved
namespace. Companion article PR: the maintainer-home-page #210.
...
```
$ dotnet tool install --tool-path /tmp/shims --add-source /tmp/feed the maintainer.the project --version 1.0.7   # OK
...
Failed to create shell shim for tool 'the project': Command 'the project' conflicts
...
`dotnet tool list` shows only the new id. Data roots survive: `~/.<app>`
...
- Live agent-instruction files (grep `.ai-badger/skills/` — a stale
  ...
  `the project-memory/SKILL.md`; other repos' `.ai-badger` copies need the same fix)
- Announcement surfaces: personal-site article + LinkedIn copy (the maintainer-home-page:
...
- Pre-existing failures: prove with a scratch worktree at origin/main
  ...
  src/the project/Models/, run the same filter) before classifying a gate failure
  as yours.
