...
Semantica is a session-scoped knowledge graph MCP server (MIT, v0.6.5+). Every MCP
invocation shares one in-memory graph — entities, relationships, and decisions accumulate
across tool calls within a session but do not survive a restart.
...
- Trivia or facts you won't query again — the graph is for active reasoning within a
  session, not a permanent store. Durable facts go to AiRaccoon (`memory_write`).
...
2. Note: there is no `import_graph` — exported graphs cannot be re-loaded.
   The graph is session-scoped only. For durable facts, write to AiRaccoon.
